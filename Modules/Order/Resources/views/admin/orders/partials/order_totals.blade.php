            ...
            <div class="table-responsive">
                <table class="table">
                    <tbody>
                        ...
                        @if (!is_null($order->installment))
                            <tr>
                                <td>Taksit <span class="coupon-code"></span></td>
                                <td class="text-right">{{ $order->installment }}</td>
                            </tr>
                        @endif

                        <tr>
                            <td>{{ trans('order::orders.total') }}</td>
                            @if(is_null($order->totalWithCommission))
                            <td class="text-right">{{ $order->total->format() }}</td>
                            @else
                                <td class="text-right">{{ \Modules\Support\Money::inCurrentCurrency($order->totalWithCommission)->format() }}</td>
