...
//            ->add($this->checkPayment());
    ...
    private function freeShipping()
    {
        return tap(new Tab('free_shipping', trans('setting::settings.tabs.free_shipping')), function (Tab $tab) {
            ...
            $tab->view('setting::admin.settings.tabs.free_shipping',['hideSaveButton' => false]);
        });
    }
    ...
    {
        return tap(new Tab('shipping_types', "Kargolar"), function (Tab $tab) {
            $company = ShippingCompany::all();
            $tab->weight(45);
            $tab->view('setting::admin.settings.shippingTypes',['company' => $company,'hideSaveButton' => true]);
        });
    }

    private function shippingTypesCreate()
    {
        return tap(new Tab('shipping_type_create', "Kargo Firması Ekle"), function (Tab $tab) {


            $tab->weight(45);
            $tab->fields(['company']);
            $tab->view('setting::admin.settings.shippingTypeCreate',['hideSaveButton' => false]);
        });
    }
